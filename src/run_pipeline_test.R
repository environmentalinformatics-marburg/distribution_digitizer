# ============================================================
# Distribution Digitizer - Full Book Processing
# ============================================================
# Purpose:
# Executes the Distribution Digitizer processing pipeline
# without the Shiny interface.
#
# Configuration is read from config/config.csv.
# ============================================================
...
colnames(config_df) <- c("key", "value")

config <- setNames(
  as.list(config_df$value),
  config_df$key
)
...
# ------------------------------------------------------------
# 3. Create NEW output directory for full processing
# ------------------------------------------------------------
...
cat("\n=== FULL BOOK RUN ===\n")
cat("New output:", pipeline_out_dir, "\n")
...
cat("Input:", config$dataInputDir, "\n")
cat("Test output:", config$dataOutputDir, "\n")
...
shapes <- lapply(shp_files, st_read, quiet = TRUE)

plot(
  st_geometry(do.call(rbind, shapes)),
  main = "Distribution Digitizer - Final polygons"
)