...
def matchtemplatetiff(tifffile, file, outdir, records, threshold):
    ...
    for pt in zip(*loc[::-1]):
        ...
        if pt[0] not in lspoint and pt[1] not in lspoint2:
            ...
            cv2.imwrite(outdir + os.path.basename(tifffile).rsplit('.', 1)[0] + os.path.basename(file).rsplit('.', 1)[0] + '_' + str(n)+ '.png', imgc[ pt[1]:(pt[1] + w), pt[0]:(pt[0] + h),:])
...
for file in glob.glob(temp + '*.png'): 
  for tifffile in glob.glob(Input+'*.png'):         
